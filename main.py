...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"data/scraper_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"),
...
def parse_arguments() -> Any:
    ...
    parser = argparse.ArgumentParser(description='Insurance Provider Data Extraction Tool')
    parser.add_argument('--provider', '-p', type=str, choices=['uhc', 'anthem', 'all'],
                        default='all', help='Provider to scrape data from')
    parser.add_argument('--zip-code', '-z', type=str, help='ZIP code to search providers for')
    parser.add_argument('--radius', '-r', type=int, default=25,
                       help='Search radius in miles (default: 25)')
    parser.add_argument('--specialty', '-s', type=str, default='mental_health',
...
def main() -> None:
    ...
    if args.provider in ['uhc', 'all']:
        ...
        try:
            uhc_scraper.scrape(zip_code=args.zip_code, radius=args.radius, 
    ...
    if args.provider in ['anthem', 'all']:
        ...
        try:
            anthem_scraper.scrape(zip_code=args.zip_code, radius=args.radius, 
                                specialty=args.specialty)
        except Exception as e:
