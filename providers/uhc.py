...
class UHCProviderScraper(BaseScraper):
    ...
    def scrape(self, zip_code: str, radius: int = 25, specialty: str = "mental_health") -> List[Dict[str, Any]]:
        ...
        logger.info(f"Starting UHC provider search for {specialty} providers in {zip_code}")
    ...
    def _select_specialty(self, specialty: str) -> None:
        ...
        try:
            ...
            professional_btn = self.wait.until(
                EC.element_to_be_clickable((By.XPATH, "//button[contains(text(), 'Medical Professional')]"))
            )
        ...
        try:
            ...
            try:
                radius_select = self.wait.until(
                    EC.element_to_be_clickable((By.XPATH, "//select[contains(@id, 'radius')]"))
                )
                ...
                radius_option = self.wait.until(
                    EC.element_to_be_clickable(
                        (By.XPATH, f"//select[contains(@id, 'radius')]/option[@value='{closest_radius}']")
                    )
                )
                radius_option.click()
            ...
            self.wait.until(
                EC.presence_of_element_located((By.CSS_SELECTOR, ".provider-info, .no-results-message"))
            )
    ...
    def _extract_providers(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            total_pages = 1
            pagination = self.driver.find_elements(By.CSS_SELECTOR, ".pagination-container")
            if pagination:
                page_elements = self.driver.find_elements(By.CSS_SELECTOR, ".pagination-container li")
            ...
            while current_page <= total_pages:
                ...
                provider_elements = self.driver.find_elements(By.CSS_SELECTOR, ".provider-info")
                ...
                if current_page < total_pages:
                    try:
                        ...
                        self.wait.until(
                            EC.presence_of_element_located((By.CSS_SELECTOR, ".provider-info"))
                        )
