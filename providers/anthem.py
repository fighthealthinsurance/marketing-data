...
class AnthemProviderScraper(BaseScraper):
    ...
    def scrape(
        ...
        logger.info(
            f"Starting Anthem provider search for {specialty} providers in {zip_code}"
        )
    ...
    def _search_as_guest(self) -> None:
        ...
        try:
            ...
            self.wait.until(
                EC.presence_of_element_located((By.CSS_SELECTOR, ".homepage"))
            )

            # Click on "Search as Guest" button
            guest_button = self.wait.until(
                EC.element_to_be_clickable(
                    (By.XPATH, "//button[contains(text(), 'Search as Guest')]")
                )
            )
            guest_button.click()
        ...
        try:
            ...
            self.wait.until(
                EC.presence_of_element_located(
                    (By.CSS_SELECTOR, ".search-suggestions .suggestion")
                )
            )
            ...
            self.wait.until(
                EC.presence_of_element_located(
                    (By.CSS_SELECTOR, ".provider-card, .no-results")
                )
            )
        ...
        try:
            ...
            if has_pagination:
                page_info = self.driver.find_element(
                    By.CSS_SELECTOR, ".pagination-info"
                ).text
            ...
            while current_page <= total_pages:
                ...
                for card in provider_cards:
                    ...
                    try:
                        expand_button = card.find_element(
                            By.CSS_SELECTOR, ".toggle-details"
                        )
                        expand_button.click()
